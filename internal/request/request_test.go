...
func TestBodyParse(t *testing.T) {
	...
	_, err = RequestFromReader(reader)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "body of the request is shorter than reported Content-Length")
	...
	reader = &chunkReader{
		data: "POST /submit HTTP/1.1\r\n" +
			...
			"Content-Length: 20\r\n" +
			"\r\n" +
			"longer content than reported content-length",
		numBytesPerRead: 3,
	}
	_, err = RequestFromReader(reader)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "body of the request is longer than reported Content-Length")
	...
	// Test: There is Body, 0 reported content length (invalid)
	reader = &chunkReader{
		data: "POST /submit HTTP/1.1\r\n" +
			"Host: localhost:42069\r\n" +
			"Content-Length: 0\r\n" +
	...
	// Test: There is Body, no reported content length (valid)
	reader = &chunkReader{
		data: "POST /submit HTTP/1.1\r\n" +
			"Host: localhost:42069\r\n" +
			"\r\n" +
